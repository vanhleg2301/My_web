...
//    }
    ...
    public void register(Account a) {
        String sql = "insert into FinalProject.dbo.[Account](Email,[Password],CustomerID,Gender,[Role]) values(?,?,?,?,2)";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, a.getEmail());
            st.setString(2, a.getPassword());
            st.setInt(3, a.getCustomerID());
            st.setBoolean(4, a.getGender());
            st.executeUpdate();            
        } catch (SQLException e) {
            System.out.println(e);
        }
...
//    }
    ...
    public Account check(String email, String password) {
        ...
        try {
            ...
            ResultSet rs = st.executeQuery();
            if (rs.next()) {
                return new Account(rs.getInt("AccountID") ,rs.getString("Email"), rs.getString("Password"), rs.getInt("Role"));
            }
        } catch (SQLException e) {
            System.out.println(e);
        }
...
//    }
    ...
    public Account getProfile(String email) {
        String sql = "SELECT [Email],[CustomerID],[Gender] FROM FinalProject.dbo.[Account] where [Email] = ?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, "Email");
            ResultSet rs = st.executeQuery();
            if (rs.next()) {
                return new Account(rs.getString(2), rs.getInt(4), rs.getBoolean(5));
            }
        } catch (SQLException e) {
            System.out.println(e);
        }
...
//        DAO p1 = new DAO();
//        String email = "vanh";
//        Account p2 = p1.getProfile(email);
...
//    }
    ...
    public Product getProductById(int id) {
        String sql = "select * from FinalProject.dbo.[Product] where CategoryID=?";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setInt(1, id);
            ResultSet rs = st.executeQuery();
            if (rs.next()) {
                return new Product(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getByte(7));
            }
        } catch (SQLException e) {
            System.out.println(e);
        }
